...
/// - [total] — the source list length **before** capping. Used by the
///   page header to surface a "Showing N of M" hint when entries have
///   been dropped.
...
RetentionCapped<T> applyRetentionCap<T>(List<T> source, int? limit) {
  if (limit == null || source.length <= limit) {
    return RetentionCapped(items: source, total: source.length);
  }
