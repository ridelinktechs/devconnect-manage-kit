...
/// Query-parameter tab. One bordered section card with a title bar
/// (icon + count badge) and a list of [ParamRow] rows underneath —
/// mirrors [HeadersTab] so both inspector tabs share the same chrome.
///
/// Per-key values: `?a=1&a=2` arrives as `["1","2"]` from
/// `Uri.queryParametersAll`. The previous build joined them with
/// `", "` which conflated commas inside a value with the value
/// boundary (same lossy pattern we just removed from the Android
/// SDK). Now each value renders on its own line with a `#N` chip
/// so a single value can still be SelectableText'd or copied whole.
...
/// One param row. Mirrors [HeaderRowCopy]: fixed-width key column,
/// expanded values column, reserved copy slot. Multi-value rows
/// render values stacked vertically with `#N` chips so a long
/// list-of-lists stays scannable instead of becoming one
/// lossy comma-joined blob.
...
class _ParamRowState extends State<ParamRow> {
  ...
  /// Returns trimmed tokens; `null` when the heuristic declines so
  /// the caller can fall back to plain `SelectableText`.
...
/// Numbered list rendering for both the `?a=1&a=2` multi-value case
/// and the auto-split single-value CSV case. Keeps the two paths
/// visually identical so a 20-item CSV reads the same way as a
/// 20-item repeated key.
